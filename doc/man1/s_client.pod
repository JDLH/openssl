...
Note that OpenSSL will also load trusted CA certificates from the default
file location: see B<-no-CAfile>.
...
Note that OpenSSL will also load trusted CA certificates from the default
directory location: see B<-no-CApath>.
...
Display the certificate chain as sent by the server. This usually includes the 
certificates of issuer(s). It probably will not include the root CA certificate, 
because the server is normally configured to not send that. 
Without this option, only the server certificate itself is displayed. 
...
B<-showcerts> option can be used to show the chain sent by the server.
